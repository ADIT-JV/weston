...
static int
gal2d_renderer_create(struct weston_compositor *ec, int use_drm)
{
    struct gal2d_renderer *gr;
    gceSTATUS status = gcvSTATUS_OK;
	gr = malloc(sizeof *gr);
...
gal2d_renderer_output_create(struct weston_output *output, NativeDisplayType display,
...

 {
    ...
    if (gr->use_drm)
    {
        ...
        if (info.stride/info.width == 2) /*16 bit*/
        {
            info.greenLength = 6;
            info.blueOffset = 0;
            info.alphaLength = 0;
        } else if (info.stride/info.width == 4)/*32 bit*/
        {
            info.greenLength = 8;
            info.blueOffset = 0;
            info.alphaLength = 8;
    ...
    if(go->nNumBuffers > 1 && !gr->use_drm)
    {
